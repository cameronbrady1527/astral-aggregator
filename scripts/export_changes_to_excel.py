#!/usr/bin/env python3
"""
Export Changes to Excel - Export change detection data to Excel format
This script reads the most recent change files for each site and exports them to an Excel workbook
with separate sheets for each site.
"""
...
class ChangesExporter:
    ...
    def get_most_recent_changes_file(self, site_name: str) -> Optional[Path]:
        """Get the most recent changes file for a site."""
    ...
    def load_changes_data(self, file_path: Path) -> Dict[str, Any]:
        """Load changes data from JSON file."""
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                data = json.load(f)
            return data
        except Exception as e:
            print(f"❌ Error loading changes file {file_path}: {e}")
            return {}
    ...
    def create_dataframe(self, changes_data: Dict[str, Any]) -> pd.DataFrame:
        """Create a pandas DataFrame from changes data."""
